...
public class GraphStatsAlpha extends
    IBiAbstractSubgraphComputation<LongWritable, LongWritable, LongWritable, Text, LongWritable, LongWritable, LongWritable> {
  ...
  public void compute(Iterable<IMessage<LongWritable, Text>> messages)
    ...
    if (getSuperstep() == 0) {
      ...
      for (IBiRemoteVertex<LongWritable, LongWritable, LongWritable, LongWritable, LongWritable> remote : getSubgraph()
          .getRemoteVertices()) {
        _neighbours.add(remote.getSubgraphId().get());
      }

      // For finding boundary Vertices
      for (IBiVertex<LongWritable, LongWritable, LongWritable, LongWritable, LongWritable> v :getSubgraph().getLocalVertices()) {
Inner:  for (IBiEdge<LongWritable, LongWritable, LongWritable, LongWritable> e : v.getOutEdges()) {
  ...
  private void cleanup() {
    ...
    IBiSubgraph<LongWritable, LongWritable, LongWritable, LongWritable, LongWritable, LongWritable> subgraph = getSubgraph();
    ...
    for (IBiVertex<LongWritable, LongWritable, LongWritable, LongWritable, LongWritable> v : getSubgraph().getVertices())
      for (IBiEdge<LongWritable, LongWritable, LongWritable, LongWritable> e : v.getInEdges())
        System.out.print("\n*********Vertex " + v.getVertexId() + " of " + getSubgraph().getSubgraphId()+ " has Inedge from " + e.getSourceVertexId() + " and sink " + e.getSinkVertexId() +"\n");

    for (IBiVertex<LongWritable, LongWritable, LongWritable, LongWritable, LongWritable> v : getSubgraph().getVertices())
      for (IBiEdge<LongWritable, LongWritable, LongWritable, LongWritable> e : v.getOutEdges())
